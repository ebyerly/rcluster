...
class RCluster:
  ...
  def __init__(self, aws_access_key_id, aws_secret_access_key, region_name,
               ...
               ip_ref = 'public', ver = 'rcluster.0.1'):
    ...
    if not key_path:
      self.key_name = ver
      key_path = os.path.join('private', self.key_name + '.pem')
      kp = self.ec2.create_key_pair(KeyName=ver)
      with open(key_path, 'w') as out:
        out.write(kp.key_material)
    else:
      self.key_name = os.path.splitext(os.path.basename(self.key_path))[0]
    
    if not 'SecurityGroups' in instance_conf:
      sg = self.ec2.create_security_group(
        GroupName=ver,
        Description='22 open to all IPs, permissive internal traffic.'
      )
      sg.authorize_ingress(IpProtocol='tcp', FromPort=22, ToPort=22,
                           CidrIp='0.0.0.0/0')
      sg.authorize_ingress(SourceSecurityGroupName = sg.group_name)
      instance_conf['SecurityGroups'] = [sg.group_name]
    
    if not 'Placement' in instance_conf:
      pg = self.ec2.create_placement_group(GroupName=ver, Strategy='cluster')
      instance_conf['Placement'] = {'GroupName' : pg.group_name}
  ...
  def createAmi(self, setup_fn, update_image = True):
    base = self.createInstances(1)[0]
    sleep(20)
    client = self.pmkConnect(base)
    conn_sftp = client.open_sftp()
    conn_sftp.put(setup_fn, 'setup.sh')
    print('Setup script {0} provided, running configuration.'.format(setup_fn))
    pmkCmd(client, 'sudo sh setup.sh')
    print('Creating AMI', self.ver)
    image = base.create_image(DryRun = False, Name = self.ver,
                              Description = "RCluster AMI", NoReboot = False)
